...
<div class="container">
    ...
    <form action="/post" method="post" enctype="multipart/form-data">
        ...
        <div class="col-md-6 col-sm-6 col-xs-6 col-md-offset-3 col-sm-offset-3 col-xs-offset-3"
            ...
            <div id="image_box" style="padding-top: 10px">
                <%--js에서 동적으로 추가--%>
                <%-- 초기 이미지 set --%>
                <c:forEach var="file" items="${files}" varStatus="vs">
                    <span id="image${vs.index}">
                        <span style="position:relative">
                            <label id="selector_label${vs.index}" for="selector${vs.index}">
                                <img src="/post/${file}" alt="이미지" width="75" height="75"
                                     class="existing_images upload_image" value="${file}">
                            </label>
                        </span>
                    </span>
                </c:forEach>
            ...
            <div class="form-group">
                ...
                <textarea class="form-control" name="article" id="article" cols="30" rows="10"
                          placeholder="게시글"><c:if test="${article != null}">${article}</c:if></textarea>
            </div>
            <div class="form-group">
                <input id="submit" class="form-control btn-info" type="submit" value="수정">
            </div>
        </div>
    </form>
...
<script type="text/javascript">
    ...
    var existingImageElements = document.getElementsByClassName('existing_images');

    for (var i = 0; i < existingImageElements.length; i++) {
        finalFiles[i] = existingImageElements[i].getAttribute("value");
    }
    ...
    function editData() {
        for (var i = 0; i < finalFiles.length; i++) {
            console.log('image file' + i + ': ' + finalFiles[i] + ', data type: ' + typeof finalFiles[i]);
        }
        ...
        for (var i = 0; i < finalFiles.length; i++) {
            if (typeof finalFiles[i] == 'object') {//새로 추가된 이미지 파일(Blob)
                formData.append('imageFile' + (i + 1), finalFiles[i]);
            } else {//기존 이미지 파일 명(string)
                formData.append('existingImage' + (i + 1), finalFiles[i]);
            }
        }
